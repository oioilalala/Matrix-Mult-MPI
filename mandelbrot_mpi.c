...
int main(int argc, char*argv[]){
    ...
    int zoom, cutoff, comm_size, comm_rank, section;
    ...
    if(MASTER == comm_rank){        // Master code
        ...
        for(int tid = 1; tid < comm_size; tid++){
            ...
            MPI_Send(&plane[0], RES * RES, MPI_INT, tid, 1, MPI_COMM_WORLD);
        }

        for(int tid = 1; tid < comm_size; tid++){
            if(RES < tid * section){           // Only populates within the dimensions of the matrix
                MPI_Recv(&plane[(tid - 1) * section],
                         section - ((tid * section) - RES),
                         MPI_INT, tid, 2, MPI_COMM_WORLD, &status);
            }
            else {
                MPI_Recv(&plane[(tid - 1) * section], section, MPI_INT, tid, 2,
        ...
        MPI_Recv(&plane[0], RES * RES, MPI_INT, MASTER, 1, MPI_COMM_WORLD, &status);
        ...
        for(int i = start; i < end; i++){
            for(int j = 0; j < RES; j++){
                plane[i * RES + j] = iteratePoint(dist * j + x_center, dist * i + y_center, cutoff);
            }
        }
        MPI_Send(&plane[start], RES * section, MPI_INT, MASTER, 2, MPI_COMM_WORLD);
    }
